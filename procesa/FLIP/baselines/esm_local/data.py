...
class FastaBatchedDataset(object):
    ...
    @classmethod
    def from_file(cls, fasta_file, trunc_len, truncate=False):
        ...
        _flush_current_seq()
        if truncate:
            sequence_strs = [q[:trunc_len] for q in sequence_strs]
    ...
    def get_batch_indices(self, toks_per_batch, extra_toks_per_seq=0):
        # iterate over sequence_strs, record length of strs, sort the lengths in ascending order
        # iterate over sorted lengths, check spare space for one more str, 
        #   if not enough space, flush current buf
        #   if enough space, adding current str into buf
        # return batches indices (NOT strs)
        #   first several batches are set of short strs
        #   last several batches are single long strs
