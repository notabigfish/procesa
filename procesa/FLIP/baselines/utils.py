...
# zs
def load_dataset(datadir, dataset, split, val_split = True, gb1_shorten=False):
    """returns dataframe of train, (val), test sets, with max_length param"""
    # datadir = Path(DATA_DIR)

    PATH = os.path.join(datadir, dataset, 'splits', split) 
...
def load_dataset_v2(datadir, dataset='hotprotein', split='s2c2_0'):
    """returns dataframe of train, test sets, with max_length param"""
    train_path = os.path.join(datadir, dataset, 'splits', f'{split}_train.csv')
    test_path = os.path.join(datadir, dataset, 'splits', f'{split}_test.csv')
    print('reading dataset:', split)

    df_train = pd.read_csv(train_path)
    df_test = pd.read_csv(test_path)
    
    df_train.sequence = df_train.sequence.apply(lambda s: re.sub(r'[^A-Z]', '', s.upper())) #remove special characters
    df_test.sequence = df_test.sequence.apply(lambda s: re.sub(r'[^A-Z]', '', s.upper())) #remove special characters

    max_length_train = max(df_train.sequence.str.len())
    max_length_test = max(df_test.sequence.str.len())
    max_length = max([max_length_train, max_length_test])

    print('loaded train/test:', len(df_train), len(df_test))
    return df_train, df_test, max_length
