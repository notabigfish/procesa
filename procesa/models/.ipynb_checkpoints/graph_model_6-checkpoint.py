...
class GraphModel6(BaseModel):
    def __init__(self,
            ...
            train_cfg=None,
            test_cfg=None):
        super(GraphModel6, self).__init__()
    ...
    def extract_feat(self, graphs):
        ...
        chunks = graphs.batch_num_nodes().tolist()
        anchor_feat = node_feat.clone()
        anchor_feat = [anchor_feat[i][:k] for i, k in enumerate(chunks)]
        anchor_feat = torch.cat(anchor_feat)
        ...
        pos_feat = node_feat.clone()
        pos_feat = [pos_feat[i][:k] for i, k in enumerate(chunks)]
        pos_feat = torch.cat(pos_feat)
        
        node_feat = self.pooling(node_feat, mask)
        
        node_feat = node_feat + seq_feat
        out = self.fc_final(node_feat)
        
        return out, pos_feat, anchor_feat
