...
class GraphModel17(BaseModel):
    def __init__(self,
            ...
            train_cfg=None,
            test_cfg=None):
        super(GraphModel17, self).__init__()
    ...
    def extract_feat(self, graphs):
        ...
        graphs.ndata['h'] =  self.ln1(self.relu(self.gat1(graphs, graphs.ndata['h']).flatten(1)))    
        graphs.ndata['h'] =  self.ln2(self.relu(self.gat2(graphs, graphs.ndata['h']).mean(1)))

        feats = torch.split(graphs.ndata['h'], graphs.batch_num_nodes().tolist(), dim=0)
